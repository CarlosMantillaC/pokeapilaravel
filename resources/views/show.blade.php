...
@section('content')
    ...
    <div class="container mx-auto py-4 px-32">
        ...
        <div id="pokemon-details" class="dark:bg-gray-400/30 bg-gray-100 p-12 rounded-xl">
            <div class="text-center">
                ...
                <div class="mt-6">
                    ...
                    <div class="flex justify-center gap-2 mt-2">
                        @foreach ($pokemon['types'] as $type)
                            <span
                                class="px-3 py-1 rounded-full text-black dark:text-white bg-{{ $type['type']['name'] }}-500">
                                {{ $type['type']['name'] }}
                            </span>
                        @endforeach
                    </div>
                ...
                <div class="mt-6">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Estadísticas:</h2>
                    <div class="space-y-2 mt-2">
                        @foreach ($pokemon['stats'] as $stat)
                            <div>
                                <p class="capitalize text-gray-800 dark:text-gray-300">
                                    {{ $stat['stat']['name'] }}: <span class="font-bold">{{ $stat['base_stat'] }}</span>
                                </p>
                                <div class="w-full bg-gray-200 dark:bg-gray-600 rounded h-2">
                                    <div class="bg-blue-500 h-2 rounded" style="width: {{ min(100, $stat['base_stat']) }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
